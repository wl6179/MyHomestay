...
<%
    ...
    selectsql = "logid,userid,iis,commentnum,topic,author,addtime,logfile,isbest,isdraft,passcheck,issave,subjectid"
...
%>
    ...
    <li class="t2" style="width:73px;float:left;">分类</li>
    <li class="t3" style="width:163px;float:left;"><%=tName%>标题</li>
    <li class="t4" style="width:68px;float:left;">作者</li>
    <li class="t5" style="width:65px;float:left;">发布时间</li>
    
    <li class="t7" style="width:52px;float:left;">状态</li>
    <li class="t9" style="width:120px;float:left;">操作</li>
    </ul>
<%do while not rs.eof %>
    <ul class="list_content tr03" style="font-size:13px;">
    <li class="t1" style="float:left;"><input name='id' type='checkbox' id="id" value='<%=cstr(rs("logid"))%>' /></li>
    <li class="t2" style="width:73px;float:left;"><%=getsubname(rs("subjectid"),allsub)%></li>
    <li class="t3" style="width:163px;float:left;">
<%if rs("logfile")<>"" then
...
Else
    ...
    <li class="t4" style="width:68px;float:left;"><%response.Write(oblog.filt_html(rs("author")))%></li>

    <li class="t5" style="width:65px;float:left;" title="<%=rs("addtime")%>"><%if rs("addtime")<>"" then  response.write formatdatetime(rs("addtime"),2) else response.write "&nbsp;"%> </li>
    
    <li class="t7" style="width:52px;float:left;"><%if rs("isdraft")=1 then response.Write("<font color=#D75372>草稿</font>") else response.Write("发布")%></li>
    <li class="t9" style="width:120px;float:left;">
<%
...
If oblog.logined_ulevel = 9 Then
	response.Write("<br />")
    If rs("isbest") = 1 Then
        response.write "<a href='user_blogmanage.asp?action=unisbest&id=" & rs("logid") & "&t=" & t & "'>取精</a>&nbsp;"
    Else
        response.write "<a href='user_blogmanage.asp?action=isbest&id=" & rs("logid") & "&t=" & t & "'>加精</a>&nbsp;"
    End If
