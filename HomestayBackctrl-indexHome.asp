...
%>
	...
	<br /><center>我的当前关注度：<%=Round(creditTemp, 2)%>%</center>
	<center>
	<% If rs("isValidate")=1 Then %>
			
		<% If rs("user_siterefu_num")<300 Then %>
		<img src="/images/MyHomestayMedal_01.gif" title="新新家庭用户" />
		<% End If %>
		<% If rs("user_siterefu_num")>300 Then %>
		<img src="/images/MyHomestayMedal_01.gif" title="成为备受关注家庭用户" />
		<% End If %>
		<% If rs("user_siterefu_num")>2000 Then %>
		<img src="/images/MyHomestayMedal_01.gif" title="访问量已达2000,您将自动成为我的住家网的热点推荐家庭用户" />
		<% End If %>
	...
	<% ElseIf rs("isValidate")=0 Then
		...
		End If %>
			...
			<td colspan="2" class="tdColspan2">我的住家网欢迎您，<%=rs("blogname")%>
			<br />
			近期活动——当您的访问量达2000时,将自动成为我的住家网站点的热点推荐家庭用户.</td>
		</tr>
	...
	'获取 [某ID] 所对应的 [某字段值]~
	 response.Write oblog.requestID_selectName("oblog_userclass","classname","where id="& rs("user_classid"))
		...
		<tr class="tr02">
			...
			<td><%
			If rs("isValidate")=1 Then
				response.Write("<font color='#038ad7'>√</font>成功通过站点审核")
			ElseIf rs("isValidate")=0 Then
				response.Write("<font color='red'>等待审核中...</font>")
			End If
			%></td>
	...
	<% End If %>
		...
		<center>
		&nbsp;<span style="font-size:8px;color:#666666;">当前封面照片</span>
		</center>
	</div>
