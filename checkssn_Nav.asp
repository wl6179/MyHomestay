...
if regusername="" or oblog.strLength(regusername)>14 or oblog.strLength(regusername)<4 then oblog.adderrstr("栏目名不能为空，也不能大于14小于4个字符！")
if oblog.chk_regname(regusername) then oblog.adderrstr("栏目名系统不允许注册！")
if oblog.chk_badword(regusername)>0 then oblog.adderrstr("栏目名中含有系统不允许的字符！")
if en_nameisnum=0 and IsNumeric(regusername) then oblog.adderrstr("栏目名不允许全部为数字！")
if oblog.chkdomain(regusername)=false then oblog.adderrstr("栏目名不合规范，只能使用小写字母，数字！")
