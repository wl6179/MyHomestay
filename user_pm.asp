...
<form action="user_pm.asp?action=save" method="post" name="oblogform">
  ...
  <br />
  <span style="font-size:12px;color:#038ad7">客户咨询热线：010-85493388/13910652850 </span>
  <ul> 
...
<div style=" width:500px;">
  ...
  <ul> 
   ...
   <ul> 
	...
	<li class="t2"><%=oblog.filt_html_b(rs("content"))%></li>
	</ul>
...
<ul>
	...
	<% Else %>
		<input type="button" onClick="window.location='user_pm.asp?action=send&incept=<%=rs("sender")%>&topic=<%="回复:"&oblog.filt_html(rs("topic"))%>'" value=" 回复 ">
	<% End If %>	
