...
sub main()
	...
	rs.Open sql,Conn,1,1
  	if rs.eof and rs.bof then
		strGuide=strGuide & " (共有0个照片)</h1><div id=""list"">"
		response.write strGuide
		showContent
	else
    	totalPut=rs.recordcount
		strGuide=strGuide & " (共有" & totalPut & "个照片)</form></h1><div id=""list"">"
		response.write strGuide
	    ...
	    if currentPage=1 then
        	showContent
        	response.write oblog.showpage(strFileName,totalput,MaxPerPage,true,true,"个照片")
			response.write "<br />"
   	 	else
   	     	if (currentPage-1)*MaxPerPage<totalPut then
...
<style>	
   ...
   <div class="list_left">
	...
	<form name="myform" method="post" action="HomestayBackctrl-PhotofilesHome.asp" onSubmit="return confirm('确定要删除选定的照片吗？');">
    <% do while not rs.eof 
		ext=rs("file_ext")%>
	<div class="filepic"><div class="fileimg"><a href='<%=rs("file_path")%>' target=_blank>
	<%'if rs("isphoto")=1 then response.Write("<img src=""images/filetype/photo.gif"" class=""fileimg"" alt=""相册文件"" />") else showfilepic(rs("file_ext"))%>
	...
	<% If rs("isUserPublicForView")=1 Then
		Response.Write("<img src='/images/home_16x16.gif' /><span style='font-size:8px;'>状态:<font color=#038ad7>公开</font></span>")
		ElseIf rs("isUserPublicForView")=0 Then
		Response.Write("<span style='font-size:8px;'>状态:<font color=red>隐藏</font></span>")
		End If
		
		If rs("isCover")=1 Then Response.Write("<font color=#038ad7>+封面</font>")
	 %>
	<br />
	
	
	<a href="?action=isCover&isCover=<%=rs("isCover")%>&upfile_isCoverPhoto_path=<%=rs("file_path")%>&id=<%=rs("fileid")%>" onClick="return confirm('确定要将该照片设为 封面照片 吗？');">
	<%
	if rs("isCover")=0 then
		response.Write("设为封面")
	elseif rs("isCover")=1 then
		response.Write("<font color=#038ad7>取消封面</font>")
	end if
	%>	
	</a>
	<br />
...
'ATFLAG
	...
	if rs("isUserPublicForView")=0 then
		response.Write("<a href='?action=PublicPhoto&isUserPublicForView="& rs("isUserPublicForView") &"&id="& rs("fileid") &"'>设为公开状态</a>")
	elseif rs("isUserPublicForView")=1 then
		response.Write("<a href='?action=PublicPhoto&isUserPublicForView="& rs("isUserPublicForView") &"&id="& rs("fileid") &"'><font color=#038ad7>取消公开状态</font></a>")
	end if
...
%>
	...
	<a href="HomestayBackctrl-PhotofilesHome.asp?action=delfile&id=<%=rs("fileid")%>" onClick="return confirm('确定要删除这个照片吗？');">删除</a>
	</div>
<%  
	i=i+1
...
%>
	...
	<input type="submit" name="Submit" value="删除选定的照片" />
	</ul>
	</form>
  </div>  
 </div><br />
...
Sub change_isCover()
	...
	If isCover= 1 Then
		...
		conn.execute("update oblog_user set isCoverPhoto=0 where userid=" & oblog.logined_uid )'WL(已设有封面图片状态为 否！)
	...
	ElseIf isCover= 0 Then
		...
		conn.execute("update oblog_user set isCoverPhoto=1 where userid=" & oblog.logined_uid )'WL(已设有封面图片状态为 是！)
	End If

	oblog.showok "相片的封面状态更新成功！",""
