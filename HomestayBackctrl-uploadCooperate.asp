...
%>
	<ul style="margin:0px;text-align: left;width:100%;"> 
      <form name="myform" method="post" action="HomestayBackctrl-uploadCooperate.asp?t=1&tMode=<%=tMode%>&re=<%=re%>&ubb=<%=ubb%>&isphoto=<%=isphoto%>" enctype="multipart/form-data">
...
'	'-----------------------------------------------------------------------------
	...
	Set Upload = New UpFile_Cls
		...
		If Upload.ErrCodes<>0 Then
			Response.write "错误："& Upload.Description & "[ <a href='HomestayBackctrl-uploadCooperate.asp?re="&re&"&ubb="&ubb&"&isphoto="&isphoto&"&tMode="& tMode &"'>重新上传</a> ]"
			Exit Sub
		End If
		If Upload.Count > 0 Then
			For Each FormName In Upload.UploadFiles
				...
				End If
					...
					Response.Write "文件："& F_FileName &"上传成功![ <a href=HomestayBackctrl-uploadCooperate.asp?re="&re&"&ubb="&ubb&"&isphoto="&isphoto&"&tMode=" & tMode &"><font color=#038ad7>再次上传</font></a> ]"
				Set File = Nothing
			Next
		Else
			Response.write "请正确选择要上传的文件。[ <a href='HomestayBackctrl-uploadCooperate.asp?re="&re&"&ubb="&ubb&"&isphoto="&isphoto&"&tMode=" & tMode &"'><font color=#038ad7>重新上传</font></a> ]"
			Exit Sub
		End If
	Set Upload = Nothing
End Sub
