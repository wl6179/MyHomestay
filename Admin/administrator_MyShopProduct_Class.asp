...
<table cellpadding="0" cellspacing="1" border="0" width="98%" class="border" align=center>
  ...
  <tr class="tdbg"> 
    ...
    <td height="30">
    	<a href="administrator_MyShopProduct_Class.asp?t=<%=t%>"><%=tName%>分类管理首页</a> | 
    	<a href="administrator_MyShopProduct_Class.asp?Action=Add&t=<%=t%>">添加<%=tName%>分类</a>&nbsp;|&nbsp;
    	<a href="administrator_MyShopProduct_Class.asp?Action=Order&t=<%=t%>">一级分类排序</a>&nbsp;|&nbsp;
    	<a href="administrator_MyShopProduct_Class.asp?Action=OrderN&t=<%=t%>">N级分类排序</a>&nbsp;|&nbsp;
    	<a href="administrator_MyShopProduct_Class.asp?Action=Reset&t=<%=t%>">复位所有<%=tName%>分类</a>&nbsp;|&nbsp;
    	<a href="administrator_MyShopProduct_Class.asp?Action=Unite&t=<%=t%>"><%=tName%>分类合并</a></td>
  </tr>
</table><br>
...
Sub main()
	...
	'sqlClass="Select * From Product_Class Where idType=" & t & " Order By RootID,OrderID"
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="border">
  <tr class="title"> 
	...
	<td height="12" align="center"><strong title="..." style="cursor:hand;">分类说明</strong></td>
  ...
  <tr class="tdbg" onMouseOut="this.style.backgroundColor=''" onMouseOver="this.style.backgroundColor='#cccccc'" style="padding: 0px 2px;"> 
	...
	If iDepth>0 Then
	  ...
	  If rsClass("Depth")=0 Then 
	  	Response.Write "<b>" 
	  End If 
	  Response.Write "<a href='administrator_MyShopProduct_Class.asp?Action=Modify&id=" & rsClass("id") & "&t=" & t & "' title='" & rsClass("ReadMe")&"["& rsClass("classid") & "]'>" & rsClass("classname") & "</a>"
	  If rsClass("Child")>0 Then 
    ...
    <td align="center">
    	...
    	<a href="administrator_MyShopProduct_Class.asp?Action=Add&ParentID=<%=rsClass("id")%>&t=<%=t%>">添加子分类</a> | 
    	<a href="administrator_MyShopProduct_Class.asp?Action=Modify&id=<%=rsClass("id")%>&t=<%=t%>">修改</a> | 
      	<a href="administrator_MyShopProduct_Class.asp?Action=Move&id=<%=rsClass("id")%>&t=<%=t%>">移动分类</a> | 
      	<a href="administrator_MyShopProduct_Class.asp?Action=Del&id=<%=rsClass("id")%>&t=<%=t%>" onClick="<%If rsClass("Child")>0 Then%>return ConfirmDel1();<%Else%>return ConfirmDel2();<%End If%>">删除</a>
...
%>
<table cellpadding="0" cellspacing="1" border="0" width="98%" class="border" align=center>
  <form name="form1" method="post" action="administrator_MyShopProduct_Class.asp?t=<%=t%>" onSubmit="return check()">  	
    <tr> 
    ...
    <tr class="tdbg"> 
      <td height="40" colspan="2" align="center"><input name="Action" type="hidden" id="Action" value="SaveAdd"> <input name="Add" type="submit" value=" 添&nbsp;&nbsp;加 " > 
        &nbsp; <input name="Cancel" type="button" id="Cancel" value=" 取 消 " onClick="window.location.href='administrator_MyShopProduct_Class.asp?t=<%=t%>'" style="cursor: hand;background-color: #cccccc;">         
      </td>
    </tr>
  </form>
</table>
...
Sub Modify()
	...
	Else
		id=CLng(id)
	End If
	sql="Select * From Product_Class Where id=" & id
	Set rsClass=Server.CreateObject ("Adodb.RecordSet")
...
<table cellpadding="0" cellspacing="1" border="0" width="98%" class="border" align=center>
  <form name="form1" method="post" action="administrator_MyShopProduct_Class.asp?t=<%=t%>" onSubmit="return check()">
    ...
    <tr class="tdbg"> 
      <td width="350"><strong>所属分类：</strong><br>
        如果你想改变所属分类，请<a href="administrator_MyShopProduct_Class.asp?Action=Move&id=<%=id%>&t=<%=t%>">点此移动分类</a></td>
      <td> 
        <%
	...
	Else
		...
		sqlParentClass="Select * From Product_Class Where id in ("& rsClass("ParentPath") &") Order By Depth"
...
%>
<table cellpadding="0" cellspacing="1" border="0" width="98%" class="border" align=center>
<form name="form1" method="post" action="administrator_MyShopProduct_Class.asp?&t=<%=t%>">
	<tr>
	...
	Else
		...
		sqlParent="Select * From Product_Class Where id in (" & rsClass("ParentPath") & ") Order By Depth"
...
Sub Order() 
	...
	sqlClass="Select * From Product_Class Where ParentID=0 And idtype="&t&" Order By RootID" 
	Set rsClass=Server.CreateObject("Adodb.RecordSet") 
	rsClass.Open sqlClass,conn,1,1 
...
<% 
	If j>1 Then 
  		Response.Write "<form action='administrator_MyShopProduct_Class.asp?Action=UpOrder&t=" & t & "' method='post'><td width='150'>" 
	...
	End If 
	If iCount>j Then 
  		Response.Write "<form action='administrator_MyShopProduct_Class.asp?Action=DownOrder&t=" & t & "' method='post'><td width='150'>" 
...
<% 
	If rsClass("ParentID")>0 Then   '如果不是一级分类，则算出相同深度的分类数目，得到该分类在相同深度的分类中所处位置（之上或者之下的分类数） 
		...
		Set trs=conn.Execute("Select Count(id) From Product_Class Where ParentID="&rsClass("ParentID")&" And OrderID<"&rsClass("OrderID")&"") 
		UpMoveNum=trs(0) 
		If isNull(UpMoveNum) Then UpMoveNum=0 
		If UpMoveNum>0 Then 
  			Response.Write "<form action='administrator_MyShopProduct_Class.asp?Action=UpOrderN&t=" & t & "' method='post'><td width='150'>" 
			Response.Write "<select name=MoveNum size=1><option value=0>向上移动</option>" 
		...
		If DownMoveNum>0 Then 
  			Response.Write "<form action='administrator_MyShopProduct_Class.asp?Action=DownOrderN&t=" & t & "'' method='post'><td width='150'>" 
			Response.Write "<select name=MoveNum size=1><option value=0>向下移动</option>" 
...
%>
<table cellpadding="0" cellspacing="1" border="0" width="98%" class="border" align=center>
  <form name="form1" method="post" action="administrator_MyShopProduct_Class.asp?Action=SaveReset&t=<%=t%>"> 
	<tr>
...
%>
<table cellpadding="0" cellspacing="1" border="0" width="98%" class="border" align=center>
<form name="myform" method="post" action="administrator_MyShopProduct_Class.asp?t=<%=t%>" onSubmit="return ConfirmUnite();">
	<tr>
...
'	End If
	...
	Set rs = conn.Execute("Select Max(id) From Product_Class")
	Maxid=rs(0)
	...
	rs.Close
	id=Maxid+1
	Set rs=conn.Execute("Select Max(rootid) From Product_Class")
	MaxRootID=rs(0)
	...
	If ParentID>0 Then
		sql="Select * From Product_Class Where id=" & ParentID & ""
		...
		Else	
			...
			If Child>0 Then		
				...
				Set rsPrevOrderID=conn.Execute("Select Max(OrderID) From Product_Class Where ParentID=" & ParentID)
				PrevOrderID=rsPrevOrderID(0)
				Set trs=conn.Execute("Select id From Product_Class Where ParentID=" & ParentID & " And OrderID=" & PrevOrderID)
				PrevID=trs(0)
				
				'得到同一父分类但比本分类级数大的子分类的最大OrderID，如果比前一个值大，则改用这个值。
				Set rsPrevOrderID=conn.Execute("Select Max(OrderID) From Product_Class Where ParentPath Like '" & ParentPath & ",%'")
	...
	Else
		If MaxRootID>0 Then
			Set trs=conn.Execute("Select id From Product_Class Where RootID=" & MaxRootID & " And Depth=0")
			PrevID=trs(0)
			trs.Close
		Else
			PrevID=0
	...
	sql="Select * From Product_Class Where ParentID=" & ParentID & " AND classname='" & classname & "'"
	...
	sql="Select top 1 * From Product_Class"
	...
	If PrevID>0 Then
		conn.Execute("Update Product_Class Set NextID=" & id & " Where id=" & PrevID)
	End If
	
	If ParentID>0 Then
		'更新其父类的子分类数
		conn.Execute("Update Product_Class Set child=child+1 Where id="&ParentID)
		
		'更新该分类排序以及大于本需要和同在本分类下的分类排序序号
		conn.Execute("Update Product_Class Set OrderID=OrderID+1 Where rootid=" & rootid & " And OrderID>" & PrevOrderID)
		conn.Execute("Update Product_Class Set OrderID=" & PrevOrderID & "+1 Where id=" & id)
	End If
	
    'Call CloseConn()
	'Response.Redirect "administrator_MyShopProduct_Class.asp?t=" & t
	oblog.showok "保存成功","administrator_MyShopProduct_Class.asp?t=" & t
End Sub
...
Sub DeleteClass()
	...
	sql="Select id,RootID,Depth,ParentID,Child,PrevID,NextID From Product_Class Where id="&id
	Set rs=Server.CreateObject ("Adodb.RecordSet")
	...
	NextID=rs("NextID")
	If rs("Depth")>0 Then
		conn.Execute("Update Product_Class Set child=child-1 Where id=" & rs("ParentID"))
	End If
	...
	If PrevID>0 Then
		conn.Execute "Update Product_Class Set NextID=" & NextID & " Where id=" & PrevID
	End If
	If NextID>0 Then
		conn.Execute "Update Product_Class Set PrevID=" & PrevID & " Where id=" & NextID
	End If
	'Call CloseConn()
	Response.redirect "administrator_MyShopProduct_Class.asp?t=" & t
...
Sub SaveMove()
	...
	If rsClass("ParentID")<>rParentID Then   '更改了所属分类，则要做一系列检查
		...
		If rsClass("ParentID")=0 Then
			If rParentID>0 Then
				Set trs=conn.Execute("Select rootid From Product_Class Where id="&rParentID)
			...
			End If
		Else
			Set trs=conn.Execute("Select id From Product_Class Where ParentPath Like '"&rsClass("ParentPath")&"," & rsClass("id") & "%' And id="&rParentID)
  ...
  Dim mrs,MaxRootID
  Set mrs=conn.Execute("Select Max(rootid) From Product_Class")
  MaxRootID=mrs(0)
  Set mrs=Nothing
  If isNull(MaxRootID) Then
	MaxRootID=0
  ...
  If clng(parentid)<>rParentID And Not (iParentID=0 And rParentID=0) Then  '假如更改了所属分类
	...
	If iParentID>0 And rParentID=0 Then  	'如果原来不是一级分类改成一级分类
		'得到上一个一级分类分类
		sql="Select id,NextID From Product_Class Where RootID=" & MaxRootID & " And Depth=0"
		Set rs=Server.CreateObject("Adodb.RecordSet")
		rs.Open sql,conn,1,3
		...
		'更新当前分类数据
		conn.Execute("Update Product_Class Set depth=0,OrderID=0,rootid="&maxrootid&",parentid=0,ParentPath='0',PrevID=" & PrevID & ",NextID=0 Where id="&id)
		'如果有下属分类，则更新其下属分类数据。下属分类的排序不需考虑，只需更新下属分类深度和一级排序ID(rootid)数据
		If child>0 Then
			i=0
			ParentPath=ParentPath & ","
			Set rs=conn.Execute("Select * From Product_Class Where ParentPath Like '%"&ParentPath & id&"%'")
			Do While Not rs.Eof
				i=i+1
				mParentPath=replace(rs("ParentPath"),ParentPath,"")
				conn.Execute("Update Product_Class Set depth=depth-"&depth&",rootid="&maxrootid&",ParentPath='"&mParentPath&"' Where id="&rs("id"))
				rs.MoveNext
			Loop
			rs.Close
			Set rs=Nothing
		...
		conn.Execute("Update Product_Class Set child=child-1 Where id="&iParentID)
		
	ElseIf iParentID>0 And rParentID>0 Then    '如果是将一个分分类移动到其他分分类下
		'得到当前分类的下属子分类数
		ParentPath=ParentPath & ","
		Set rs=conn.Execute("Select Count(*) From Product_Class Where ParentPath Like '%"&ParentPath & id&"%'")
		ClassCount=rs(0)
		...
		'获得目标分类的相关信息		
		Set trs=conn.Execute("Select * From Product_Class Where id="&rParentID)
		If trs("Child")>0 Then		
			'得到与本分类同级的最后一个分类的OrderID
			Set rsPrevOrderID=conn.Execute("Select Max(OrderID) From Product_Class Where ParentID=" & trs("id"))
			PrevOrderID=rsPrevOrderID(0)
			'得到与本分类同级的最后一个分类的id
			sql="Select id,NextID From Product_Class Where ParentID=" & trs("id") & " And OrderID=" & PrevOrderID
			Set rs=Server.createobject("Adodb.RecordSet")
			rs.Open sql,conn,1,3
			PrevID=rs(0)    '得到新的PrevID
			...
			'得到同一父分类但比本分类级数大的子分类的最大OrderID，如果比前一个值大，则改用这个值。
			Set rsPrevOrderID=conn.Execute("Select Max(OrderID) From Product_Class Where ParentPath Like '" & trs("ParentPath") & "," & trs("id") & ",%'")
			If (Not(rsPrevOrderID.Bof And rsPrevOrderID.Eof)) Then
				If Not IsNull(rsPrevOrderID(0))  Then
			 		If rsPrevOrderID(0)>PrevOrderID Then
						PrevOrderID=rsPrevOrderID(0)
		...
		conn.Execute("Update Product_Class Set OrderID=OrderID+" & ClassCount & "+1 Where rootid=" & trs("rootid") & " And OrderID>" & PrevOrderID)
		
		'更新当前分类数据
		conn.Execute("Update Product_Class Set depth="&trs("depth")&"+1,OrderID="&PrevOrderID&"+1,rootid="&trs("rootid")&",ParentID="&rParentID&",ParentPath='" & trs("ParentPath") & "," & trs("id") & "',PrevID=" & PrevID & ",NextID=0 Where id="&id)
		
		'如果有子分类则更新子分类数据，深度为原来的相对深度加上当前所属分类的深度
		Set rs=conn.Execute("Select * From Product_Class Where ParentPath Like '%"&ParentPath&id&"%' Order By OrderID")
		i=1
		Do While Not rs.Eof
			i=i+1
			iParentPath=trs("ParentPath") & "," & trs("id") & "," & replace(rs("ParentPath"),ParentPath,"")
			conn.Execute("Update Product_Class Set depth=depth-"&depth&"+"&trs("depth")&"+1,OrderID="&PrevOrderID&"+"&i&",rootid="&trs("rootid")&",ParentPath='"&iParentPath&"' Where id="&rs("id"))
			rs.MoveNext
		Loop
		rs.Close
		Set rs=Nothing
		...
		'更新所指向的上级分类的子分类数
		conn.Execute("Update Product_Class Set child=child+1 Where id="&rParentID)
		
		'更新其原父类的子分类数			
		conn.Execute("Update Product_Class Set child=child-1 Where id="&iParentID)
	Else    '如果原来是一级分类改成其他分类的下属分类
		'得到移动的分类总数
		Set rs=conn.Execute("Select Count(*) From Product_Class Where rootid="&rootid)
		ClassCount=rs(0)
		rs.Close
		Set rs=Nothing
		...
		conn.Execute("Update Product_Class Set PrevID=" & PrevID & ",NextID=0 Where id=" & id)
		Set rs=conn.Execute("Select * From Product_Class Where rootid="&rootid&" Order By OrderID")
		i=0
		Do While Not rs.Eof
			i=i+1
			If rs("parentid")=0 Then
				ParentPath=trs("ParentPath") & "," & trs("id")
				conn.Execute("Update Product_Class Set depth=depth+"&trs("depth")&"+1,OrderID="&PrevOrderID&"+"&i&",rootid="&trs("rootid")&",ParentPath='"&ParentPath&"',parentid="&rParentID&" Where id="&rs("id"))
			Else
				ParentPath=trs("ParentPath") & "," & trs("id") & "," & replace(rs("ParentPath"),"0,","")
				conn.Execute("Update Product_Class Set depth=depth+"&trs("depth")&"+1,OrderID="&PrevOrderID&"+"&i&",rootid="&trs("rootid")&",ParentPath='"&ParentPath&"' Where id="&rs("id"))
			End If
			rs.MoveNext
		Loop
		rs.Close
...
Sub UpOrder()
	...
	'得到本分类的PrevID,NextID
	Set rs=conn.Execute("Select PrevID,NextID From Product_Class Where id=" & id)
	PrevID=rs(0)
	NextID=rs(1)
	rs.Close
	Set rs=Nothing
	...
	MaxRootID=mrs(0)+1
	'先将当前分类移至最后，包括子分类
	conn.Execute("Update Product_Class Set RootID=" & MaxRootID & " Where RootID=" & cRootID)
	
	'然后将位于当前分类以上的分类的RootID依次加一，范围为要提升的数字
	sqlOrder="Select * From Product_Class Where ParentID=0 And RootID<" & cRootID & " Order By RootID desc"
	...
	Do While Not rsOrder.Eof
		...
		If i>MoveNum Then
			rsOrder("PrevID")=id
			rsOrder.Update
			conn.Execute("Update Product_Class Set NextID=" & rsOrder("id") & " Where id=" & id)
			conn.Execute("Update Product_Class Set RootID=RootID+1 Where RootID=" & tRootID)
			Exit Do
		End If
		conn.Execute("Update Product_Class Set RootID=RootID+1 Where RootID=" & tRootID)
		rsOrder.MoveNext
	...
	rsOrder.MoveNext
	If rsOrder.Eof Then
		conn.Execute("Update Product_Class Set PrevID=0 Where id=" & id)
	Else
		rsOrder("NextID")=id
		rsOrder.Update
		conn.Execute("Update Product_Class Set PrevID=" & rsOrder("id") & " Where id=" & id)
	End If	
	rsOrder.Close
	Set rsOrder=Nothing
	
	'然后再将当前分类从最后移到相应位置，包括子分类
	conn.Execute("Update Product_Class Set RootID=" & tRootID & " Where RootID=" & MaxRootID)
	'Call CloseConn()
	Response.Redirect "administrator_MyShopProduct_Class.asp?Action=Order&t=" & t
End Sub
...
Sub DownOrder()
	...
	sqlOrder="Select * From Product_Class Where ParentID=0 And idType =" & t & " And RootID>" & cRootID & " Order By RootID"
	Set rsOrder=Server.CreateObject("Adodb.RecordSet")
	...
	rsOrder.MoveNext
	If rsOrder.Eof Then
		conn.Execute("Update Product_Class Set NextID=0 Where id=" & id)
	Else
		rsOrder("PrevID")=id
		rsOrder.Update
		conn.Execute("Update Product_Class Set NextID=" & rsOrder("id") & " Where id=" & id)
...
Sub UpOrderN()
	...
	'要移动的分类信息
	Set rs=conn.Execute("Select ParentID,OrderID,ParentPath,child,PrevID,NextID From Product_Class Where id="&id)
	ParentID=rs(0)
	OrderID=rs(1)
	ParentPath=rs(2) & "," & id
	child=rs(3)
	...
	Set rs=Nothing
	If child>0 Then
		Set rs=conn.Execute("Select Count(*) From Product_Class Where ParentPath Like '%"&ParentPath&"%'")
		oldorders=rs(0)
		rs.Close
		Set rs=Nothing
	Else
	...
	sql="Select id,OrderID,child,ParentPath,PrevID,NextID From Product_Class Where ParentID="&ParentID&" And OrderID<"&OrderID&" Order By OrderID desc"
	Set rs=Server.CreateObject("Adodb.RecordSet")
	rs.Open sql,conn,1,3
	i=1
	Do While Not rs.Eof
		...
		If rs(2)>0 Then
			ii=i+1
			Set trs=conn.Execute("Select id,OrderID From Product_Class Where ParentPath Like '%"&rs(3)&","&rs(0)&"%' Order By OrderID")
			If Not (trs.Eof And trs.Bof) Then
				Do While Not trs.Eof
					conn.Execute("Update Product_Class Set OrderID="&tOrderID+oldorders+ii&" Where id="&trs(0))
					ii=ii+1
					trs.MoveNext
				Loop
			End If
		...
		If i>MoveNum Then
			rs(4)=id
			rs.Update
			conn.Execute("Update Product_Class Set NextID=" & rs(0) & " Where id=" & id)
			conn.Execute("Update Product_Class Set OrderID="&tOrderID+oldorders+i-1&" Where id="&rs(0))		
			Exit Do
		End If
		conn.Execute("Update Product_Class Set OrderID="&tOrderID+oldorders+i-1&" Where id="&rs(0))
		rs.MoveNext
	...
	Else
		rs(5)=id
		rs.Update
		conn.Execute("Update Product_Class Set PrevID=" & rs(0) & " Where id=" & id)
	End If	
	rs.Close
	Set rs=Nothing
	
	'更新所要排序的分类的序号
	conn.Execute("Update Product_Class Set OrderID="&tOrderID&" Where id="&id)
	'如果有下属分类，则更新其下属分类排序
	If child>0 Then
		i=1
		Set rs=conn.Execute("Select id From Product_Class Where ParentPath Like '%"&ParentPath&"%' Order By OrderID")
		Do While Not rs.Eof
			conn.Execute("Update Product_Class Set OrderID="&tOrderID+i&" Where id="&rs(0))
			i=i+1
			rs.MoveNext
		Loop
		rs.Close
	...
	End If
	'Call CloseConn()
	Response.Redirect "administrator_MyShopProduct_Class.asp?Action=OrderN&t=" & t
End Sub
...
Sub DownOrderN()
	...
	sql="Select id,OrderID,child,ParentPath,PrevID,NextID From Product_Class Where ParentID="&ParentID&" And OrderID>"&OrderID&" Order By OrderID"
	Set rs=Server.CreateObject("Adodb.RecordSet")
	rs.Open sql,conn,1,3
	i=0      '同级分类
	ii=0     '同级分类和子分类
	Do While Not rs.Eof
		'conn.Execute("Update Product_Class Set OrderID="&OrderID+ii&" Where id="&rs(0))
		If rs(2)>0 Then
			Set trs=conn.Execute("Select id,OrderID From Product_Class Where ParentPath Like '%"&rs(3)&","&rs(0)&"%' Order By OrderID")
			If Not (trs.Eof And trs.Bof) Then
				Do While Not trs.Eof
					ii=ii+1
					conn.Execute("Update Product_Class Set OrderID="&OrderID+ii&" Where id="&trs(0))
					trs.MoveNext
				Loop
			End If
			trs.Close
		...
		If i>=MoveNum Then
			...
			rs.Update
			conn.Execute("Update Product_Class Set PrevID=" & rs(0) & " Where id=" & id)
			conn.Execute("Update Product_Class Set OrderID="&OrderID+ii-1&" Where id="&rs(0))		
			Exit Do
		End If
		conn.Execute("Update Product_Class Set OrderID="&OrderID+ii-1&" Where id="&rs(0))
		rs.MoveNext
	...
	'更新所要排序的分类的序号
	conn.Execute("Update Product_Class Set OrderID="&OrderID+ii&" Where id="&id)
	'如果有下属分类，则更新其下属分类排序
	If child>0 Then
		i=1
		Set rs=conn.Execute("Select id From Product_Class Where ParentPath Like '%"&ParentPath&"%' Order By OrderID")
		Do While Not rs.Eof
			conn.Execute("Update Product_Class Set OrderID="&OrderID+ii+i&" Where id="&rs(0))
			i=i+1
			rs.MoveNext
		Loop
		rs.Close
...
Sub SaveReset()
	...
	sql="Select id From Product_Class Where idType=" & t & " Order By RootID,OrderID"
	Set rs=Server.CreateObject("Adodb.RecordSet")
	rs.Open sql,conn,1,1
	...
	Do While Not rs.Eof
		...
		End If
		rs.moveprevious
		conn.Execute("Update Product_Class Set RootID=" & i & ",OrderID=0,ParentID=0,Child=0,ParentPath='0',Depth=0,PrevID=" & PrevID & ",NextID=" & NextID & " Where id=" & rs(0))
		PrevID=rs(0)
		i=i+1
		rs.MoveNext
	Loop
	...
	Response.Write "复位成功！请返回<a href='administrator_MyShopProduct_Class.asp?t=" & t & "'>分类管理首页</a>做分类的归属设置。"
End Sub
...
Sub SaveUnite()
	...
	Set rs=conn.Execute("Select Child From Product_Class Where id=" & Targetid)
	If rs.Bof And rs.Eof Then
		FoundErr=True
	...
	'得到当前分类信息
	Set rs=conn.Execute("Select id,ParentID,ParentPath,PrevID,NextID,Depth From Product_Class Where id="&id)
	iParentID=rs(1)
	Depth=rs(5)
	If iParentID=0 Then
		ParentPath=rs(0)
	...
	'判断是否是合并到其下属分类中
	Set rs=conn.Execute("Select id From Product_Class Where id="&Targetid&" And ParentPath Like '"&ParentPath&"%'")
	...
	'得到当前分类的下属分类ID
	Set rs=conn.Execute("Select id From Product_Class Where ParentPath Like '"&ParentPath&"%'")
	i=0
	If Not (rs.Eof And rs.Bof) Then
		Do While Not rs.Eof
	...
	'删除被合并分类及其下属分类
	conn.Execute("Delete From Product_Class Where id in ("&ParentPath&")")
...
'
'	If isDisplay= 1 Then
'		conn.Execute("Update Product_Class Set isDisplay=0 Where classid=" & classid )
'	ElseIf isDisplay= 0 Then
'		conn.Execute("Update Product_Class Set isDisplay=1 Where classid=" & classid )
'	End If
'
'	Response.redirect "administrator_MyShopProduct_Class.asp"
'		
'End Sub
'
'
...
'
'	
'		conn.Execute("Update Product_Class Set userid_Nav=0 Where classid=" & id )
'
'
'	Response.redirect "administrator_MyShopProduct_Class.asp"
'		
