...
strFileName="admin_userCooperate.asp?UserSearch=" & UserSearch
if strField<>"" then
...
<table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" Class="border">
  <tr class="topbg"> 
    <td height="22" colspan=2 align=center><strong>加 盟 用 户 管 理</strong></td>
  </tr>
  <form name="form1" action="admin_userCooperate.asp" method="get">
    <tr class="tdbg"> 
      ...
      <td width="687" height="30"><select size=1 name="UserSearch" onChange="javascript:submit()">
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;<a href="admin_userCooperate.asp">合作伙伴用户管理首页</a>&nbsp;|&nbsp;<a href="/RegisterCooperate.asp" target="_blank">添加新用户</a></td>
    </tr>
  </form>
  <form name="form2" method="post" action="admin_userCooperate.asp">
  <tr class="tdbg">
    ...
    <td >
	  ...
	  <option value="blogname" >真实姓名称</option>
...
sub main()
	dim strGuide
	strGuide="<table width='100%'><tr><td align='left'>您现在的位置：<a href='admin_userCooperate.asp'>注册合作伙伴用户管理</a>&nbsp;&gt;&gt;&nbsp;"
	select case UserSearch
		case 0
			sql="select top 500 * from oblog_user where user_level<=6 And user_level_team=1 order by UserID desc"
			strGuide=strGuide & "最后注册的500个合作伙伴"
		case 1
			sql="select top 100 * from oblog_user where user_level<=6 And user_level_team=1 order by log_count desc"
			strGuide=strGuide & "发布文章最多的前100个用户"
		case 2
			sql="select top 100 * from  oblog_user where user_level<=6 And user_level_team=1 order by log_count"
			strGuide=strGuide & "发布文章最少的100个用户"
		case 3
			sql="select * from  oblog_user where user_level<=6 And user_level_team=1 and user_level=8 order by userid desc"
			strGuide=strGuide & "所有VIP用户"
		case 4
			sql="select * from  oblog_user where user_level<=6 And user_level_team=1 and user_level=9 order by userid desc"
			strGuide=strGuide & "前台管理员"
		case 5
			sql="select * from  oblog_user where user_level<=6 And user_level_team=1 and user_isbest=1 order by userid desc"
			strGuide=strGuide & "推荐用户"
		case 6
			sql="select * from oblog_user where user_level<=6 And user_level_team=1 and User_Level=6 order by userID  desc"
			strGuide=strGuide & "等待管理认证证的用户"
		case 7
			sql="select * from oblog_user where user_level<=6 And user_level_team=1 and  LockUser =1 order by userID  desc"
			...
			if Keyword="" then
				...
				case "UserID"
					if IsNumeric(Keyword)=false then
						...
						sql="select * from oblog_user where user_level<=6 And user_level_team=1 and userID =" & Clng(Keyword)
						strGuide=strGuide & "用户ID等于<font color=red> " & Clng(Keyword) & " </font>的用户"
					end if
				case "UserName"
					if is_sqldata=1 then
						sql="select * from oblog_user where user_level<=6 And user_level_team=1 and username like '%" & Keyword & "%' order by userID  desc"
						strGuide=strGuide & "用户名中含有“ <font color=red>" & Keyword & "</font> ”的用户"
					else
						sql="select * from oblog_user where user_level<=6 And user_level_team=1 and username= '" & Keyword & "' order by userID  desc"
						strGuide=strGuide & "用户名等于“ <font color=red>" & Keyword & "</font> ”的用户"
					end if
					
				case "nickname"
					if is_sqldata=1 then
						sql="select * from oblog_user where user_level<=6 And user_level_team=1 and nickname like '%" & Keyword & "%' order by userID  desc"
						strGuide=strGuide & "用户昵称中含有“ <font color=red>" & Keyword & "</font> ”的用户"
					else
						sql="select * from oblog_user where user_level<=6 And user_level_team=1 and nickname='" & Keyword & "' order by userID  desc"
						strGuide=strGuide & "用户昵称等于“ <font color=red>" & Keyword & "</font> ”的用户"
					end if
				case "ip"
					if is_sqldata=1 then
						sql="select * from oblog_user where user_level<=6 And user_level_team=1 and lastloginip like '%" & Keyword & "%' order by userID  desc"
						strGuide=strGuide & "最后登录ip中含有“ <font color=red>" & Keyword & "</font> ”的用户"
					else
						sql="select * from oblog_user where user_level<=6 And user_level_team=1 and lastloginip='" & Keyword & "' order by userID  desc"
						strGuide=strGuide & "最后登录ip等于“ <font color=red>" & Keyword & "</font> ”的用户"
					end if
				case "blogname"
					if is_sqldata=1 then
						sql="select * from oblog_user where user_level<=6 And user_level_team=1 and blogname like '%" & Keyword & "%' order by userID  desc"
						strGuide=strGuide & "真实姓名中含有“ <font color=red>" & Keyword & "</font> ”的用户"
					else
						sql="select * from oblog_user where user_level<=6 And user_level_team=1 and blogname='" & Keyword & "' order by userID  desc"
						strGuide=strGuide & "真实姓名等于“ <font color=red>" & Keyword & "</font> ”的用户"
					end if
				case "logcount"
					sql="select top 500 * from oblog_user where user_level<=6 And user_level_team=1 and log_count < " & clng(Keyword) & " order by userID  desc"
					strGuide=strGuide & "文章数小于“ <font color=red>" & Keyword & "</font> ”的用户"
				case "logintimes"
					sql="select top 500 * from oblog_user where user_level<=6 And user_level_team=1 and logintimes < " & clng(Keyword) & " order by userID  desc"
					strGuide=strGuide & "登录次数小于“ <font color=red>" & Keyword & "</font> ”的用户"
				case "lastlogintime"
				   If is_sqldata=1 Then 
					sql="select top 500 * from oblog_user where user_level<=6 And user_level_team=1 and datediff(d,lastlogintime,getdate())>"&clng(Keyword)&" order by userID  desc"
				   Else 
				   	sql="select top 500 * from oblog_user where user_level<=6 And user_level_team=1 and datediff('d',lastlogintime,now())>"&clng(Keyword)&" order by userID  desc"
				   End If 
...
<table width='98%' border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
  <form name="myform" method="Post" action="admin_userCooperate.asp" onSubmit="return confirm('确定要执行选定的操作吗？');">
     <td>
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#FFFFFF" class="border">
          <tr class="title"> 
            ...
            <td height="22" align="center"><font color="#FFFFFF">真实姓名</font></td>
            <td height="22" align="center"><font color="#FFFFFF">公司名称</font></td>
          ...
          <tr class="tdbg" onMouseOut="this.style.backgroundColor=''" onMouseOver="this.style.backgroundColor='#BFDFFF'"> 
			...
			response.write "<a href='admin_userCooperate.asp?Action=Modify&UserID=" & rs("userID") & "'>" & rs("userName") & "</a>"
			%> </td>
            <td align="center"> <%
			Response.Write rs("blogname")
			'''select case rs("User_Level")
'''				case 6
'''					If rs("user_level_team")= 0 Then
'''					response.write "<font color=green>中国家庭用户</font>"
'''					ElseIf rs("user_level_team")= 1 Then
'''					response.write "<font color=#038ad7>合作伙伴用户</font>"
'''					End If
'''				case 7
...
'''	end if
	...
	%> </td>
	  ...
	  %></td>
			...
			response.write "<a href='admin_userCooperate.asp?Action=isValidate&isValidate="& rs("isValidate") &"&UserID=" & rs("userID") & "' title='将显示在接待家庭栏目中！'><font color=green>"
			If rs("isValidate")=1 Then
				Response.Write("<font color=green title='已经通过了站点审核'>通过</font>")
			ElseIf rs("isValidate")=0 Then
				Response.Write("<font color=red title='此加盟商未通过站点审核，不接受其提交的外教信息！'>待审核</font>")
			End If
			Response.Write "</font></a>&nbsp;"
			
			
		response.write "<a href='admin_userCooperate.asp?Action=Modify&UserID=" & rs("userID") & "'>修改</a>&nbsp;"
		response.write "<a href='admin_userCooperate.asp?Action=gouser2&username=" & rs("username") & "' target='blank'>进后台</a>&nbsp;"
		if rs("LockUser")=0 then
			response.write "<br /><a href='admin_userCooperate.asp?Action=Lock&UserID=" & rs("userID") & "'>锁定</a>&nbsp;"
		else
            response.write "<br /><a href='admin_userCooperate.asp?Action=UnLock&UserID=" & rs("userID") & "'>解锁</a>&nbsp;"
		end if
        response.write "<a href='admin_userCooperate.asp?Action=Del&UserID=" & rs("userID") & "' onClick='return confirm(""确定要删除此用户吗？"");'>删除</a>&nbsp;"
		%> </td>
          </tr>
          <%
	i=i+1
...
%>
<FORM name="Form1" action="admin_userCooperate.asp" method="post">
  <table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#FFFFFF" class="border">
    ...
    <TR style="display:none;" class="tdbg" > 
      ...
      <TD width="60%"><select name="User_Level" id="User_Level">
          <option value="6" <%if clng(rsUser("user_level"))=6 then response.Write("selected")%>>中国家庭or合作伙伴用户</option>
...
sub UpdateUser()
%>
<FORM name="Form1" action="admin_userCooperate.asp?action=DoUpdate" method="post">
  <table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#FFFFFF" class="border">
...
<FORM name="Form1" action="admin_userCooperate.asp?action=DoUpdatelog" method="post">
  <table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#FFFFFF" class="border">
    <tr align="center" class="title"> 
...
%>
<FORM name="Form1" action="admin_userCooperate.asp?action=gouser2" method="post" target="_blank">
  <table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#FFFFFF" class="border">
    <tr align="center" class="title"> 
...
sub gouser2()
	...
	if not rs.eof then
		...
		set rs=nothing
		response.Redirect("../HomestayBackctrl-indexCooperate.asp")
	else
		set rs=nothing
...
sub DelUser()
	...
	response.redirect "admin_userCooperate.asp"
end sub
...
Sub change_isValidate()'设置中国家庭是否通过我站点审核...通过便可以在站点显示，成为公开家庭用户.
	...
	oblog.showok "保存更新","admin_userCooperate.asp"
