...
strFileName = "admin_userCooperate_PassTeacherDeleted.asp?t=" & t
If usersearch = "" Then
    usersearch = 0
Else
    usersearch = CLng(usersearch)
    strFileName = "admin_userCooperate_PassTeacherDeleted.asp?usersearch=" & usersearch & "&t=" & t
End If
If selectsub <> "" Then
    selectsub = CLng(selectsub)
    strFileName = "admin_userCooperate_PassTeacherDeleted.asp?usersearch=10&selectsub=" & selectsub & "&t=" & t
Else
    selectsub = 0
End If
If Keyword <> "" Then
    strFileName="admin_userCooperate_PassTeacherDeleted.asp?usersearch=10&keyword="&Keyword&"&Field="&strField & "&t="  & t
End If
...
<table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" Class="border">
  <tr class="topbg"> 
    <td height="22" colspan=2 align=center><strong>被删除的 外教资料管理</strong></td>
  </tr>
  <form name="form1" action="admin_userCooperate_PassTeacherDeleted.asp" method="get">
    <tr class="tdbg"> 
	  ...
	  <select size=1 name="usersearch" onChange="javascript:submit()">
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;<a href="admin_userCooperate_PassTeacherDeleted.asp">被删除外教资料管理首页</a></td>
    </tr>
  </form>
  
  
  <form name="form2" method="post" action="admin_userCooperate_PassTeacherDeleted.asp">
  <tr class="tdbg">
...
elseif Action="isAdminDeleted" then
    	Call change_isAdminDeleted
...
sub main()
	dim strGuide
	strGuide="<table width='100%'><tr><td align='left'>您现在的位置：<a href='admin_userCooperate_PassTeacherDeleted.asp'>被删除外教资料管理</a>&nbsp;&gt;&gt;&nbsp;"
	select case UserSearch
		case 0
			sql="select top 500 * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>100 order by addtime desc"
			strGuide=strGuide & "最后的500个被删除的外教资料"
		case 1'列出没有发布URL的外教资料；
			sql="select top 500 * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>100 And logPublic_path='' order by addtime desc"
			strGuide=strGuide & "没有填写发布地址URL的前500个外教资料"
		case 2
			sql="select top 500 * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>100 And logPublic_path<>'' order by addtime desc"
			strGuide=strGuide & "已经填写了发布地址URL的前500个外教资料"
		case 3
			sql="select top 500 * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>10000 order by addtime desc"
			strGuide=strGuide & "尚保存在加盟用户草稿箱里的前500个外教资料"
		case 4
			sql="select * from  [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>100 order by addtime desc"
			strGuide=strGuide & "强制查看已被删除的前500个外教资料"
		case 5
			sql="select * from  [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>100 and user_isbest=1 order by addtime desc"
			strGuide=strGuide & "推荐用户"
		case 6
			sql="select * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>100 and User_Level=6 order by userID  desc"
			strGuide=strGuide & "等待管理认证证的用户"
		case 7
			sql="select * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>100 and  LockUser =1 order by userID  desc"
			...
			if Keyword="" then
				...
				case "logid"
					if IsNumeric(Keyword)=false then
						...
						sql="select * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>10000 and logid =" & Clng(Keyword)
						strGuide=strGuide & "外教资料ID等于<font color=red> " & Clng(Keyword) & " </font>的外教资料"
					end if
				case "topic"
					if is_sqldata=1 then
						sql="select * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>10000 and topic like '%" & Keyword & "%' order by addtime  desc"
						strGuide=strGuide & "外教资料的标题注释中含有“ <font color=red>" & Keyword & "</font> ”的外教资料"
					else
						sql="select * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>10000 and topic= '" & Keyword & "' order by addtime  desc"
						strGuide=strGuide & "外教资料的标题注释等于“ <font color=red>" & Keyword & "</font> ”的外教资料"
					end if
					
				case "author"
					if is_sqldata=1 then
						sql="select * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>10000 and author like '%" & Keyword & "%' order by addtime  desc"
						strGuide=strGuide & "加盟帐户名含有：“ <font color=red>" & Keyword & "</font> ”的帐号 所填写的所有外教资料！"
					else
						sql="select * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>10000 and author='" & Keyword & "' order by addtime  desc"
						strGuide=strGuide & "加盟帐户等于“ <font color=red>" & Keyword & "</font> ”的帐号 所填写的所有外教资料"
					end if
				case "logtext"
					if is_sqldata=1 then
						sql="select * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>10000 and logtext like '%" & Keyword & "%' order by addtime desc"
						strGuide=strGuide & "提交内容中含有“ <font color=red>" & Keyword & "</font> ”的外教资料"
					else
						sql="select * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>10000 and logtext='" & Keyword & "' order by addtime  desc"
				...
				case "blogname"
					if is_sqldata=1 then
						sql="select * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>100 and blogname like '%" & Keyword & "%' order by userID  desc"
						strGuide=strGuide & "真实姓名中含有“ <font color=red>" & Keyword & "</font> ”的用户"
					else
						sql="select * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>100 and blogname='" & Keyword & "' order by userID  desc"
						strGuide=strGuide & "真实姓名等于“ <font color=red>" & Keyword & "</font> ”的用户"
					end if
				case "logcount"
					sql="select top 500 * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>100 and log_count < " & clng(Keyword) & " order by userID  desc"
					strGuide=strGuide & "文章数小于“ <font color=red>" & Keyword & "</font> ”的用户"
				case "logintimes"
					sql="select top 500 * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>100 and logintimes < " & clng(Keyword) & " order by userID  desc"
					strGuide=strGuide & "登录次数小于“ <font color=red>" & Keyword & "</font> ”的用户"
				case "lastlogintime"
				   If is_sqldata=1 Then 
					sql="select top 500 * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>100 and datediff(d,lastlogintime,getdate())>"&clng(Keyword)&" order by userID  desc"
				   Else 
				   	sql="select top 500 * from [oblog_logCooperateSubmit] where isDelete=1 And state_shenhe<>100 And isdraft<>100 and datediff('d',lastlogintime,now())>"&clng(Keyword)&" order by userID  desc"
				   End If 
...
<table width='98%' border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
  <form name="myform" method="Post" action="admin_userCooperate_PassTeacherDeleted.asp" onSubmit="return confirm('确定要执行选定的操作吗？');">
     <td>
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#FFFFFF" class="border">
	...
	%> </td>
			...
			<% If rs("state_shenhe")=0 Then %>
			<input type="button" value="通过审核" onClick="if (confirm('请先预览资料信息，并进行关联发布地址的操作，才能正式通过审核！') == false)return false;window.location.href='admin_userCooperate_PassTeacherDeleted.asp?action=state_shenhe&state_shenhe=<%=rs("state_shenhe")%>&logPublic_path=<%=rs("logPublic_path")%>&logid=<%=rs("logid")%>&userid=<%=rs("userid")%>'">
			<% ElseIf rs("state_shenhe")=1 Then %>
			<input type="button" value=" 取 消 " onClick="if (confirm('确定要取消该外教资料的审核状态吗？') == false)return false;window.location.href='admin_userCooperate_PassTeacher.asp?action=state_shenhe&state_shenhe=<%=rs("state_shenhe")%>&logPublic_path=<%=rs("logPublic_path")%>&logid=<%=rs("logid")%>&userid=<%=rs("userid")%>'">
			<% End If %>
			</td>
			
            <td width="150" align="center"><%
		response.write "<a href='admin_userCooperate_PassTeacherDeleted.asp?Action=gouser2&userid=" & rs("userid") & "&logid="& rs("logid") &"&actionTemp=preview' target='blank'>预览资料</a>&nbsp;"
		
		If rs("logPublic_path")<>"" And Len(rs("logPublic_path"))>6 Then
			response.write "<a href='admin_userCooperate_PassTeacherDeleted.asp?Action=Modify&logid=" & rs("logid") & "' title='"& rs("logPublic_path") &"'><font color=#038ad7>已关联URL√</font></a>&nbsp;"
		Else
			response.write "<a href='admin_userCooperate_PassTeacherDeleted.asp?Action=Modify&logid=" & rs("logid") & "' title='请在审核之前，先关联好此外教的站点发布地址URL！'><font color=red>请关联发布地址</font></a>&nbsp;"
		End If
		
		response.write "<br /><a href='admin_userCooperate_PassTeacherDeleted.asp?Action=gouser2&userid=" & rs("userid") & "&logid="& rs("logid") &"' title='您可以强制修改加盟用户提交的外教资料内容' target='blank'>强制修改</a>&nbsp;"
		
        
		%> 
		<% If rs("isAdminDeleted")=1 Then %>
			<input title="此外教资料是大后台监控删除的，可以恢复此外教资料到添加其的加盟商的资料管理列表里！(加盟用户自己删除的将不可以恢复)" style="cursor:hand;" type="button" value="恢复删除" onClick="if (confirm('确认要恢复此外教资料到加盟商的资料管理列表里吗？') == false)return false;window.location.href='admin_userCooperate_PassTeacherDeleted.asp?action=isAdminDeleted&logid=<%=rs("logid")%>'">
...
%>
<FORM name="Form1" action="admin_userCooperate_PassTeacherDeleted.asp" method="post">
...
sub UpdateUser()
%>
<FORM name="Form1" action="admin_userCooperate_PassTeacherDeleted.asp?action=DoUpdate" method="post">
  <table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#FFFFFF" class="border">
...
<FORM name="Form1" action="admin_userCooperate_PassTeacherDeleted.asp?action=DoUpdatelog" method="post">
  <table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#FFFFFF" class="border">
    <tr align="center" class="title"> 
...
%>
<FORM name="Form1" action="admin_userCooperate_PassTeacherDeleted.asp?action=gouser2" method="post" target="_blank">
  <table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#FFFFFF" class="border">
    <tr align="center" class="title"> 
...
sub DelUser()
	...
	response.redirect "admin_userCooperate_PassTeacherDeleted.asp"
end sub
...
Dim TempWord
	...
	oblog.showok TempWord,"admin_userCooperate_PassTeacherDeleted.asp"
	
End Sub



Sub change_isAdminDeleted()
	dim sql,rs,id
	...
	if FoundErr=True then
		...
		conn.execute("update oblog_logCooperateSubmit set isAdminDeleted=0 where logid=" & id )
		
		conn.execute("update oblog_logCooperateSubmit set isDelete=0 where logid=" & id )

	oblog.showok "已恢复到添加此外教资料的加盟商的资料管理列表中了！","admin_userCooperate_PassTeacherDeleted.asp"
