...
<table cellpadding="0" cellspacing="1" border="0" width="98%" class="border" align=center>
  <tr>
	<td colspan="2" align="center" class="title"><strong>首页变换广告管理</strong></td>
  </tr>
  <tr class="tdbg"> 
    <td width="80" height="30"><strong>管理导航：</strong></td>
    <td height="30">
    	<a href="admin_logclass_IndexAdMapsControl.asp?t=<%=t%>">首页变换广告管理首页</a> | 
    	<a href="admin_logclass_IndexAdMapsControl.asp?Action=Add&t=<%=t%>">添加广告图片</a>
		</td>
  </tr>
</table><br>
...
sub main()
	...
	'sqlClass="select * From oblog_IndexAdMapsControl Where idType=" & t & " order by RootID,OrderID"
	sqlClass="Select classid,id,classname,classlognum,ordernum,orderid,parentid,parentpath,"&_
				 ...
				 "From oblog_IndexAdMapsControl As logclass "&_
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="border">
  <tr class="title"> 
    <td height="22" align="center"><strong>图片名称</strong></td>
	<td height="12" align="center"><strong title="请在 栏目空间管理+发布 的 ‘绑定分类’处进行绑定！" style="cursor:hand;">链接指向地址</strong></td>
...
%>
  <tr class="tdbg" onMouseOut="this.style.backgroundColor=''" onMouseOver="this.style.backgroundColor='#cccccc'" style="padding: 0px 2px;"> 
    <td> <%
	If rsClass("EName")<>"" And Len(rsClass("EName"))>6 Then 
	Response.Write "<img src='"& rsClass("EName") &"' width=50 height=60 />"
	Else
	...
	if iDepth>0 then
	  ...
	  response.write "<a href='admin_logclass_IndexAdMapsControl.asp?Action=Modify&id=" & rsClass("id") & "&t=" & t & "' title='" & rsClass("ReadMe") & "'>" & rsClass("classname") & "</a>"
	  if rsClass("Child")>0 then 
	  	response.write "（" & rsClass("Child") & "）" 
	  end if 
	  
	  
	  Response.write "<br /><font color=#666666 style='font-weight:normal;'>&nbsp;&nbsp;[&nbsp;"& rsClass("EName") &"&nbsp;]</font>"
	  Response.write "<font color=#038ad7>[0"& ii &"]</font>"
    ...
    <td align="center">
    	...
    	<a href="admin_logclass_IndexAdMapsControl.asp?Action=Modify&id=<%=rsClass("id")%>&t=<%=t%>">修改</a> | 
      	
      	<a href="admin_logclass_IndexAdMapsControl.asp?Action=Del&id=<%=rsClass("id")%>&t=<%=t%>" onClick="<%if rsClass("Child")>0 then%>return ConfirmDel1();<%else%>return ConfirmDel2();<%end if%>">删除</a>
...
%>
<table cellpadding="0" cellspacing="1" border="0" width="98%" class="border" align=center>
  <form name="form1" method="post" action="admin_logclass_IndexAdMapsControl.asp?t=<%=t%>" onSubmit="return check()">  	
    <tr> 
    ...
    <tr class="tdbg"> 
      <td width="350"><strong>图片名称：</strong></td>
      <td><input name="classname" type="text" size="37" maxlength="20"></td>
    </tr>
	<tr class="tdbg"> 
      <td width="350"><font color="blue"><strong>图片来源地址url：</strong></font></td>
      <td><input name="EName" type="text" size="37" maxlength="100"></td>
    </tr>
    <tr class="tdbg"> 
      <td width="350"><strong>说明文字：<br>
        </strong> 鼠标移至图片名称上时将显示设定的说明文字（不支持HTML）</td>
    ...
    <tr class="tdbg"> 
      <td height="40" colspan="2" align="center"><input name="Action" type="hidden" id="Action" value="SaveAdd"> <input name="Add" type="submit" value=" 添&nbsp;&nbsp;加 " > 
        &nbsp; <input name="Cancel" type="button" id="Cancel" value=" 取 消 " onClick="window.location.href='admin_logclass_IndexAdMapsControl.asp?t=<%=t%>'" style="cursor: hand;background-color: #cccccc;">         
      </td>
    </tr>
  </form>
</table>
...
sub Modify()
	...
	sql="select * From oblog_IndexAdMapsControl where id=" & id
	set rsClass=server.CreateObject ("Adodb.recordset")
...
<table cellpadding="0" cellspacing="1" border="0" width="98%" class="border" align=center>
  <form name="form1" method="post" action="admin_logclass_IndexAdMapsControl.asp?t=<%=t%>" onSubmit="return check()">
    ...
    <tr class="tdbg"> 
      <td width="350"><strong>所属分类：</strong><br>
        如果你想改变所属分类，请<a href="admin_logclass_IndexAdMapsControl.asp?Action=Move&id=<%=id%>&t=<%=t%>">点此移动分类</a></td>
      <td> 
        <%
	if rsClass("ParentID")<=0 then
		...
		sqlParentClass="Select * From oblog_IndexAdMapsControl where id in (" & rsClass("ParentPath") & ") order by Depth"
	...
	<tr class="tdbg"> 
      <td width="350"><font color="blue"><strong>图片来源地址url：</strong></font></td>
      <td><input name="EName" type="text" value="<%=rsClass("EName")%>" size="37" maxlength="100"></td>
    </tr>
    <tr class="tdbg"> 
      <td width="350"><strong>说明文字：<br>
        </strong> 鼠标移至图片名称上时将显示设定的说明文字（不支持HTML）</td>
...
%>
<table cellpadding="0" cellspacing="1" border="0" width="98%" class="border" align=center>
<form name="form1" method="post" action="admin_logclass_IndexAdMapsControl.asp?&t=<%=t%>">
	<tr>
	...
	<tr class="tdbg"> 
      <td width="200"><strong>图片来源地址url：</strong></td>
      <td><%=rsClass("EName")%></td>
    </tr>
    <tr class="tdbg">
	...
	if rsClass("ParentID")<=0 then
		...
		sqlParent="Select * From oblog_IndexAdMapsControl where id in (" & rsClass("ParentPath") & ") order by Depth"
...
sub Order() 
	...
	sqlClass="select * From oblog_IndexAdMapsControl where ParentID=0 and idtype="&t&" order by RootID" 
	set rsClass=server.CreateObject("adodb.recordset") 
	rsClass.open sqlClass,conn,1,1 
...
<% 
	if j>1 then 
  		response.write "<form action='admin_logclass_IndexAdMapsControl.asp?Action=UpOrder&t=" & t & "' method='post'><td width='150'>" 
	...
	if iCount>j then 
  		response.write "<form action='admin_logclass_IndexAdMapsControl.asp?Action=DownOrder&t=" & t & "' method='post'><td width='150'>" 
...
<% 
	if rsClass("ParentID")>0 then   '如果不是一级分类，则算出相同深度的分类数目，得到该分类在相同深度的分类中所处位置（之上或者之下的分类数） 
		...
		set trs=conn.execute("select count(id) From oblog_IndexAdMapsControl where ParentID="&rsClass("ParentID")&" and OrderID<"&rsClass("OrderID")&"") 
		UpMoveNum=trs(0) 
		if isnull(UpMoveNum) then UpMoveNum=0 
		if UpMoveNum>0 then 
  			response.write "<form action='admin_logclass_IndexAdMapsControl.asp?Action=UpOrderN&t=" & t & "' method='post'><td width='150'>" 
			response.write "<select name=MoveNum size=1><option value=0>向上移动</option>" 
		...
		if DownMoveNum>0 then 
  			response.write "<form action='admin_logclass_IndexAdMapsControl.asp?Action=DownOrderN&t=" & t & "'' method='post'><td width='150'>" 
			response.write "<select name=MoveNum size=1><option value=0>向下移动</option>" 
...
%>
<table cellpadding="0" cellspacing="1" border="0" width="98%" class="border" align=center>
  <form name="form1" method="post" action="admin_logclass_IndexAdMapsControl.asp?Action=SaveReset&t=<%=t%>"> 
	<tr>
...
%>
<table cellpadding="0" cellspacing="1" border="0" width="98%" class="border" align=center>
<form name="myform" method="post" action="admin_logclass_IndexAdMapsControl.asp?t=<%=t%>" onSubmit="return ConfirmUnite();">
	<tr>
...
sub SaveAdd()
	...
	if classname="" then
		response.Write "<br><li>图片名称不能为空！</li>"
		response.End()
	end if
	...
	set rs = conn.execute("select Max(id) From oblog_IndexAdMapsControl")
	Maxid=rs(0)
	...
	rs.close
	id=Maxid+1
	set rs=conn.execute("select max(rootid) From oblog_IndexAdMapsControl")
	MaxRootID=rs(0)
	...
	if ParentID>0 then
		sql="select * From oblog_IndexAdMapsControl where id=" & ParentID & ""
		...
		if FoundErr=True then
			...
			if Child>0 then		
				...
				set rsPrevOrderID=conn.execute("select Max(OrderID) From oblog_IndexAdMapsControl where ParentID=" & ParentID)
				PrevOrderID=rsPrevOrderID(0)
				set trs=conn.execute("select id From oblog_IndexAdMapsControl where ParentID=" & ParentID & " and OrderID=" & PrevOrderID)
				PrevID=trs(0)
				
				'得到同一父分类但比本分类级数大的子分类的最大OrderID，如果比前一个值大，则改用这个值。
				set rsPrevOrderID=conn.execute("select Max(OrderID) From oblog_IndexAdMapsControl where ParentPath like '" & ParentPath & ",%'")
		...
		if MaxRootID>0 then
			set trs=conn.execute("select id From oblog_IndexAdMapsControl where RootID=" & MaxRootID & " and Depth=0")
			PrevID=trs(0)
			trs.close
		else
			PrevID=0
	...
	sql="Select * From oblog_IndexAdMapsControl Where ParentID=" & ParentID & " AND classname='" & classname & "'"
	...
	sql="Select top 1 * From oblog_IndexAdMapsControl"
	...
	if PrevID>0 then
		conn.execute("update oblog_IndexAdMapsControl set NextID=" & id & " where id=" & PrevID)
	end if
	
	if ParentID>0 then
		'更新其父类的子分类数
		conn.execute("update oblog_IndexAdMapsControl set child=child+1 where id="&ParentID)
		
		'更新该分类排序以及大于本需要和同在本分类下的分类排序序号
		conn.execute("update oblog_IndexAdMapsControl set OrderID=OrderID+1 where rootid=" & rootid & " and OrderID>" & PrevOrderID)
		conn.execute("update oblog_IndexAdMapsControl set OrderID=" & PrevOrderID & "+1 where id=" & id)
	end if
	
    'call CloseConn()
	'Response.Redirect "admin_logclass_IndexAdMapsControl.asp?t=" & t
	oblog.showok "保存成功","admin_logclass_IndexAdMapsControl.asp?t=" & t
end sub
...
sub DeleteClass()
	...
	sql="select id,RootID,Depth,ParentID,Child,PrevID,NextID From oblog_IndexAdMapsControl where id="&id
	set rs=server.CreateObject ("Adodb.recordset")
	...
	NextID=rs("NextID")
	if rs("Depth")>0 then
		conn.execute("update oblog_IndexAdMapsControl set child=child-1 where id=" & rs("ParentID"))
	end if
	rs.delete
	rs.update
	rs.close
	...
	if PrevID>0 then
		conn.execute "update oblog_IndexAdMapsControl set NextID=" & NextID & " where id=" & PrevID
	end if
	if NextID>0 then
		conn.execute "update oblog_IndexAdMapsControl set PrevID=" & PrevID & " where id=" & NextID
	end if
	'call CloseConn()
	response.redirect "admin_logclass_IndexAdMapsControl.asp?t=" & t
...
sub SaveMove()
	...
	if rsClass("ParentID")<>rParentID then   '更改了所属分类，则要做一系列检查
		...
		if rsClass("ParentID")=0 then
			if rParentID>0 then
				set trs=conn.execute("select rootid From oblog_IndexAdMapsControl where id="&rParentID)
			...
			set trs=conn.execute("select id From oblog_IndexAdMapsControl where ParentPath like '"&rsClass("ParentPath")&"," & rsClass("id") & "%' and id="&rParentID)
  ...
  dim mrs,MaxRootID
  set mrs=conn.execute("select max(rootid) From oblog_IndexAdMapsControl")
  MaxRootID=mrs(0)
  set mrs=nothing
  if isnull(MaxRootID) then
	MaxRootID=0
  ...
  if clng(parentid)<>rParentID and not (iParentID=0 and rParentID=0) then  '假如更改了所属分类
	...
	if iParentID>0 and rParentID=0 then  	'如果原来不是一级分类改成一级分类
		'得到上一个一级分类分类
		sql="select id,NextID From oblog_IndexAdMapsControl where RootID=" & MaxRootID & " and Depth=0"
		set rs=server.CreateObject("Adodb.recordset")
		rs.open sql,conn,1,3
		...
		'更新当前分类数据
		conn.execute("update oblog_IndexAdMapsControl set depth=0,OrderID=0,rootid="&maxrootid&",parentid=0,ParentPath='0',PrevID=" & PrevID & ",NextID=0 where id="&id)
		'如果有下属分类，则更新其下属分类数据。下属分类的排序不需考虑，只需更新下属分类深度和一级排序ID(rootid)数据
		if child>0 then
			i=0
			ParentPath=ParentPath & ","
			set rs=conn.execute("select * From oblog_IndexAdMapsControl where ParentPath like '%"&ParentPath & id&"%'")
			do while not rs.eof
				i=i+1
				mParentPath=replace(rs("ParentPath"),ParentPath,"")
				conn.execute("update oblog_IndexAdMapsControl set depth=depth-"&depth&",rootid="&maxrootid&",ParentPath='"&mParentPath&"' where id="&rs("id"))
				rs.movenext
			loop
			rs.close
			set rs=nothing
		...
		conn.execute("update oblog_IndexAdMapsControl set child=child-1 where id="&iParentID)
		
	elseif iParentID>0 and rParentID>0 then    '如果是将一个分分类移动到其他分分类下
		'得到当前分类的下属子分类数
		ParentPath=ParentPath & ","
		set rs=conn.execute("select count(*) From oblog_IndexAdMapsControl where ParentPath like '%"&ParentPath & id&"%'")
		ClassCount=rs(0)
		...
		'获得目标分类的相关信息		
		set trs=conn.execute("select * From oblog_IndexAdMapsControl where id="&rParentID)
		if trs("Child")>0 then		
			'得到与本分类同级的最后一个分类的OrderID
			set rsPrevOrderID=conn.execute("select Max(OrderID) From oblog_IndexAdMapsControl where ParentID=" & trs("id"))
			PrevOrderID=rsPrevOrderID(0)
			'得到与本分类同级的最后一个分类的id
			sql="select id,NextID From oblog_IndexAdMapsControl where ParentID=" & trs("id") & " and OrderID=" & PrevOrderID
			set rs=server.createobject("adodb.recordset")
			rs.open sql,conn,1,3
			PrevID=rs(0)    '得到新的PrevID
			...
			'得到同一父分类但比本分类级数大的子分类的最大OrderID，如果比前一个值大，则改用这个值。
			set rsPrevOrderID=conn.execute("select Max(OrderID) From oblog_IndexAdMapsControl where ParentPath like '" & trs("ParentPath") & "," & trs("id") & ",%'")
			if (not(rsPrevOrderID.bof and rsPrevOrderID.eof)) then
				if not IsNull(rsPrevOrderID(0))  then
			 		if rsPrevOrderID(0)>PrevOrderID then
						PrevOrderID=rsPrevOrderID(0)
		...
		conn.execute("update oblog_IndexAdMapsControl set OrderID=OrderID+" & ClassCount & "+1 where rootid=" & trs("rootid") & " and OrderID>" & PrevOrderID)
		
		'更新当前分类数据
		conn.execute("update oblog_IndexAdMapsControl set depth="&trs("depth")&"+1,OrderID="&PrevOrderID&"+1,rootid="&trs("rootid")&",ParentID="&rParentID&",ParentPath='" & trs("ParentPath") & "," & trs("id") & "',PrevID=" & PrevID & ",NextID=0 where id="&id)
		
		'如果有子分类则更新子分类数据，深度为原来的相对深度加上当前所属分类的深度
		set rs=conn.execute("select * From oblog_IndexAdMapsControl where ParentPath like '%"&ParentPath&id&"%' order by OrderID")
		i=1
		do while not rs.eof
			i=i+1
			iParentPath=trs("ParentPath") & "," & trs("id") & "," & replace(rs("ParentPath"),ParentPath,"")
			conn.execute("update oblog_IndexAdMapsControl set depth=depth-"&depth&"+"&trs("depth")&"+1,OrderID="&PrevOrderID&"+"&i&",rootid="&trs("rootid")&",ParentPath='"&iParentPath&"' where id="&rs("id"))
			rs.movenext
		loop
		rs.close
		set rs=nothing
		...
		'更新所指向的上级分类的子分类数
		conn.execute("update oblog_IndexAdMapsControl set child=child+1 where id="&rParentID)
		
		'更新其原父类的子分类数			
		conn.execute("update oblog_IndexAdMapsControl set child=child-1 where id="&iParentID)
	else    '如果原来是一级分类改成其他分类的下属分类
		'得到移动的分类总数
		set rs=conn.execute("select count(*) From oblog_IndexAdMapsControl where rootid="&rootid)
		ClassCount=rs(0)
		rs.close
		set rs=nothing
		...
		conn.execute("update oblog_IndexAdMapsControl set PrevID=" & PrevID & ",NextID=0 where id=" & id)
		set rs=conn.execute("select * From oblog_IndexAdMapsControl where rootid="&rootid&" order by OrderID")
		i=0
		do while not rs.eof
			i=i+1
			if rs("parentid")=0 then
				ParentPath=trs("ParentPath") & "," & trs("id")
				conn.execute("update oblog_IndexAdMapsControl set depth=depth+"&trs("depth")&"+1,OrderID="&PrevOrderID&"+"&i&",rootid="&trs("rootid")&",ParentPath='"&ParentPath&"',parentid="&rParentID&" where id="&rs("id"))
			else
				ParentPath=trs("ParentPath") & "," & trs("id") & "," & replace(rs("ParentPath"),"0,","")
				conn.execute("update oblog_IndexAdMapsControl set depth=depth+"&trs("depth")&"+1,OrderID="&PrevOrderID&"+"&i&",rootid="&trs("rootid")&",ParentPath='"&ParentPath&"' where id="&rs("id"))
			end if
			rs.movenext
		loop
		rs.close
...
sub UpOrder()
	...
	'得到本分类的PrevID,NextID
	set rs=conn.execute("select PrevID,NextID From oblog_IndexAdMapsControl where id=" & id)
	PrevID=rs(0)
	NextID=rs(1)
	rs.close
	set rs=nothing
	...
	MaxRootID=mrs(0)+1
	'先将当前分类移至最后，包括子分类
	conn.execute("update oblog_IndexAdMapsControl set RootID=" & MaxRootID & " where RootID=" & cRootID)
	
	'然后将位于当前分类以上的分类的RootID依次加一，范围为要提升的数字
	sqlOrder="select * From oblog_IndexAdMapsControl where ParentID=0 and RootID<" & cRootID & " order by RootID desc"
	...
	do while not rsOrder.eof
		...
		if i>MoveNum then
			rsOrder("PrevID")=id
			rsOrder.update
			conn.execute("update oblog_IndexAdMapsControl set NextID=" & rsOrder("id") & " where id=" & id)
			conn.execute("update oblog_IndexAdMapsControl set RootID=RootID+1 where RootID=" & tRootID)
			exit do
		end if
		conn.execute("update oblog_IndexAdMapsControl set RootID=RootID+1 where RootID=" & tRootID)
		rsOrder.movenext
	...
	rsOrder.movenext
	if rsOrder.eof then
		conn.execute("update oblog_IndexAdMapsControl set PrevID=0 where id=" & id)
	else
		rsOrder("NextID")=id
		rsOrder.update
		conn.execute("update oblog_IndexAdMapsControl set PrevID=" & rsOrder("id") & " where id=" & id)
	end if	
	rsOrder.close
	set rsOrder=nothing
	
	'然后再将当前分类从最后移到相应位置，包括子分类
	conn.execute("update oblog_IndexAdMapsControl set RootID=" & tRootID & " where RootID=" & MaxRootID)
	'call CloseConn()
	response.Redirect "admin_logclass_IndexAdMapsControl.asp?Action=Order&t=" & t
end sub
...
sub DownOrder()
	...
	sqlOrder="select * From oblog_IndexAdMapsControl where ParentID=0 and idType =" & t & " And RootID>" & cRootID & " order by RootID"
	set rsOrder=server.CreateObject("adodb.recordset")
	...
	rsOrder.movenext
	if rsOrder.eof then
		conn.execute("update oblog_IndexAdMapsControl set NextID=0 where id=" & id)
	else
		rsOrder("PrevID")=id
		rsOrder.update
		conn.execute("update oblog_IndexAdMapsControl set NextID=" & rsOrder("id") & " where id=" & id)
...
sub UpOrderN()
	...
	'要移动的分类信息
	set rs=conn.execute("select ParentID,OrderID,ParentPath,child,PrevID,NextID From oblog_IndexAdMapsControl where id="&id)
	ParentID=rs(0)
	OrderID=rs(1)
	ParentPath=rs(2) & "," & id
	child=rs(3)
	...
	set rs=nothing
	if child>0 then
		set rs=conn.execute("select count(*) From oblog_IndexAdMapsControl where ParentPath like '%"&ParentPath&"%'")
		oldorders=rs(0)
		rs.close
		set rs=nothing
	else
	...
	sql="select id,OrderID,child,ParentPath,PrevID,NextID From oblog_IndexAdMapsControl where ParentID="&ParentID&" and OrderID<"&OrderID&" order by OrderID desc"
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,3
	i=1
	do while not rs.eof
		...
		if rs(2)>0 then
			ii=i+1
			set trs=conn.execute("select id,OrderID From oblog_IndexAdMapsControl where ParentPath like '%"&rs(3)&","&rs(0)&"%' order by OrderID")
			if not (trs.eof and trs.bof) then
				do while not trs.eof
					conn.execute("update oblog_IndexAdMapsControl set OrderID="&tOrderID+oldorders+ii&" where id="&trs(0))
					ii=ii+1
					trs.movenext
				loop
			end if
		...
		if i>MoveNum then
			rs(4)=id
			rs.update
			conn.execute("update oblog_IndexAdMapsControl set NextID=" & rs(0) & " where id=" & id)
			conn.execute("update oblog_IndexAdMapsControl set OrderID="&tOrderID+oldorders+i-1&" where id="&rs(0))		
			exit do
		end if
		conn.execute("update oblog_IndexAdMapsControl set OrderID="&tOrderID+oldorders+i-1&" where id="&rs(0))
		rs.movenext
	...
	if rs.eof then
		...
		rs(5)=id
		rs.update
		conn.execute("update oblog_IndexAdMapsControl set PrevID=" & rs(0) & " where id=" & id)
	end if	
	rs.close
	set rs=nothing
	
	'更新所要排序的分类的序号
	conn.execute("update oblog_IndexAdMapsControl set OrderID="&tOrderID&" where id="&id)
	'如果有下属分类，则更新其下属分类排序
	if child>0 then
		i=1
		set rs=conn.execute("select id From oblog_IndexAdMapsControl where ParentPath like '%"&ParentPath&"%' order by OrderID")
		do while not rs.eof
			conn.execute("update oblog_IndexAdMapsControl set OrderID="&tOrderID+i&" where id="&rs(0))
			i=i+1
			rs.movenext
		loop
		rs.close
	...
	'call CloseConn()
	response.Redirect "admin_logclass_IndexAdMapsControl.asp?Action=OrderN&t=" & t
end sub
...
sub DownOrderN()
	...
	sql="select id,OrderID,child,ParentPath,PrevID,NextID From oblog_IndexAdMapsControl where ParentID="&ParentID&" and OrderID>"&OrderID&" order by OrderID"
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,3
	i=0      '同级分类
	ii=0     '同级分类和子分类
	do while not rs.eof
		'conn.execute("update oblog_IndexAdMapsControl set OrderID="&OrderID+ii&" where id="&rs(0))
		if rs(2)>0 then
			set trs=conn.execute("select id,OrderID From oblog_IndexAdMapsControl where ParentPath like '%"&rs(3)&","&rs(0)&"%' order by OrderID")
			if not (trs.eof and trs.bof) then
				do while not trs.eof
					ii=ii+1
					conn.execute("update oblog_IndexAdMapsControl set OrderID="&OrderID+ii&" where id="&trs(0))
					trs.movenext
				loop
			end if
			trs.close
		...
		if i>=MoveNum then
			...
			rs.update
			conn.execute("update oblog_IndexAdMapsControl set PrevID=" & rs(0) & " where id=" & id)
			conn.execute("update oblog_IndexAdMapsControl set OrderID="&OrderID+ii-1&" where id="&rs(0))		
			exit do
		end if
		conn.execute("update oblog_IndexAdMapsControl set OrderID="&OrderID+ii-1&" where id="&rs(0))
		rs.movenext
	...
	'更新所要排序的分类的序号
	conn.execute("update oblog_IndexAdMapsControl set OrderID="&OrderID+ii&" where id="&id)
	'如果有下属分类，则更新其下属分类排序
	if child>0 then
		i=1
		set rs=conn.execute("select id From oblog_IndexAdMapsControl where ParentPath like '%"&ParentPath&"%' order by OrderID")
		do while not rs.eof
			conn.execute("update oblog_IndexAdMapsControl set OrderID="&OrderID+ii+i&" where id="&rs(0))
			i=i+1
			rs.movenext
		loop
		rs.close
...
sub SaveReset()
	...
	sql="select id From oblog_IndexAdMapsControl Where idType=" & t & " order by RootID,OrderID"
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	...
	do while not rs.eof
		...
		rs.moveprevious
		conn.execute("update oblog_IndexAdMapsControl set RootID=" & i & ",OrderID=0,ParentID=0,Child=0,ParentPath='0',Depth=0,PrevID=" & PrevID & ",NextID=" & NextID & " where id=" & rs(0))
		PrevID=rs(0)
		i=i+1
		rs.movenext
	loop
	...
	response.Write "复位成功！请返回<a href='admin_logclass_IndexAdMapsControl.asp?t=" & t & "'>首页变换广告管理首页</a>做分类的归属设置。"
end sub
...
sub SaveUnite()
	...
	set rs=conn.execute("select Child From oblog_IndexAdMapsControl where id=" & Targetid)
	if rs.bof and rs.eof then
		FoundErr=True
	...
	'得到当前分类信息
	set rs=conn.execute("select id,ParentID,ParentPath,PrevID,NextID,Depth From oblog_IndexAdMapsControl where id="&id)
	iParentID=rs(1)
	Depth=rs(5)
	if iParentID=0 then
		ParentPath=rs(0)
	...
	'判断是否是合并到其下属分类中
	set rs=conn.execute("select id From oblog_IndexAdMapsControl where id="&Targetid&" and ParentPath like '"&ParentPath&"%'")
	...
	'得到当前分类的下属分类ID
	set rs=conn.execute("select id From oblog_IndexAdMapsControl where ParentPath like '"&ParentPath&"%'")
	i=0
	if not (rs.eof and rs.bof) then
		do while not rs.eof
	...
	'删除被合并分类及其下属分类
	conn.execute("delete From oblog_IndexAdMapsControl where id in ("&ParentPath&")")
...
Sub isDisplay()
	...
	If isDisplay= 1 Then
		conn.execute("update oblog_IndexAdMapsControl set isDisplay=0 where classid=" & classid )
	ElseIf isDisplay= 0 Then
		conn.execute("update oblog_IndexAdMapsControl set isDisplay=1 where classid=" & classid )
	End If

	response.redirect "admin_logclass_IndexAdMapsControl.asp"
...
Sub ClearBinding()'强制清除绑定状态；
	...
	if FoundErr=True then
		...
		conn.execute("Update oblog_IndexAdMapsControl Set userid_Nav=0 Where classid=" & id )
