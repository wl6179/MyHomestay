  ...
  <form name="form1" action="admin_user.asp" method="get">
    <tr class="tdbg"> 
      ...
      <td width="687" height="30"><select size=1 name="UserSearch" onChange="javascript:submit()">
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;<a href="admin_user.asp">用户管理首页</a>&nbsp;|&nbsp;<a href="../reg.asp" target="_blank">添加新用户</a></td>
    </tr>
  </form>
...
sub main()
	...
	select case UserSearch
		...
		case 3
			sql="select * from  oblog_user where user_level<=6 and user_level=8 order by userid desc"
			strGuide=strGuide & "所有VIP用户"
		case 4
			sql="select * from  oblog_user where user_level<=6 and user_level=9 order by userid desc"
			strGuide=strGuide & "前台管理员"
		case 5
			sql="select * from  oblog_user where user_level<=6 and user_isbest=1 order by userid desc"
			strGuide=strGuide & "推荐用户"
		case 6
			sql="select * from oblog_user where user_level<=6 and User_Level=6 order by userID  desc"
			strGuide=strGuide & "等待管理认证证的用户"
		case 7
			sql="select * from oblog_user where user_level<=6 and  LockUser =1 order by userID  desc"
...
sub MoveUser()
	...
	if instr(UserID,",")>0 then
		...
		if User_Level=6 then
			msg="&nbsp;&nbsp;&nbsp;&nbsp;已经成功将选定用户设为“<font color=blue>注册的家庭用户</font>”！"
