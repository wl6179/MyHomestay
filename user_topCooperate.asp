...
'''response.write("index.asp?fromurl="&replace(oblog.GetUrl,"&","$"))
if request("t")="change0" then change(0)
if request("t")="change1" then change(1)
'判断分栏
if oblog.logined_uframe=1 then
	cssfile="back-ctrl.css"
	if actiontype="" and (instr(nowurl,"HomestayBackctrl-indexCooperate.asp") and actiontype<>"main") then 
		call frame():response.End()
	end if
	'if instr(nowurl,"HomestayBackctrl-indexCooperate.asp")=0 and instr(nowurl,"user_top.asp")=0 then
	'	response.Redirect("HomestayBackctrl-indexCooperate.asp?url="&nowurl)
	'end if
else
...
sub utop()%>
	...
	<title>
      <%=oblog.logined_uname%> - 我的住家网 加盟用户帐号后台管理
    </title>
...
sub frame()
	if url="" or instr(url,"HomestayBackctrl-indexCooperate.asp") then url="HomestayBackctrl-indexCooperate.asp?actiontype=main"
	with response
...
function chkframe()
	if oblog.logined_uframe=1 then
		chkframe="<a href=""HomestayBackctrl-indexCooperate.asp?t=change0"">切换全屏</a>"
	else
		chkframe="<a href=""HomestayBackctrl-indexCooperate.asp?t=change1"">切换分栏</a>"
	end if
end function
...
sub change(s)
	...
	set rs=nothing
	if s=1 then
		response.Write("<script language=JavaScript>top.location='HomestayBackctrl-indexCooperate.asp?url="&oblog.comeurl&"';</script>")
	else
		response.Write("<script language=JavaScript>top.location='"&oblog.comeurl&"';</script>")
	end if
end sub
