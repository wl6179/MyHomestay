  ...
  <!--<div class="submenu">
	...
	<div class="submenu_content">
		...
		If Request("ubb")="1" Then
			%>
		&nbsp; <a href="user_post.asp?ubb=0&t=<%=t%>">切换到html方式添加<%=tName%></a>
		<%
		Else
		%>
		&nbsp; <a href="user_post.asp?ubb=1&t=<%=t%>">切换到ubb方式添加<%=tName%>(firefox推荐)</a>
		<%End If%>
...
sub main()
	...
	Dim logPhoto_AddressURL'WL;
	Dim isTcomehere,TcomehereDate,TName,TcomehereHowLongTime,TcomehereWhither,TNationality 'WL;
	...
	if logid>0 then
		if oblog.logined_ulevel=9 then
			...
			set rs=oblog.execute("select * from oblog_log where logid="&logid&" and ( userid="&oblog.logined_uid&"or authorid="&oblog.logined_uid&" )")
		end if
		if rs.eof then 
			set rs=nothing
		...
		logPhoto_AddressURL=oblog.filt_html(rs("logPhoto_AddressURL"))
...
<div id="list">
	<!--<form action="user_post.asp?action=savelog&t=<%=t%>" method="post" name="oblogform" id="oblogform" onSubmit="return VerifySubmit()">-->
	<form action="user_post.asp?action=savelog&t=<%=t%>" method="post" name="oblogform" id="oblogform" onSubmit="return VerifySubmit();">
    <ul class="list_edit"><%=tName%>标题： 
	<input name="topic" type=text id="topic" size="50" maxlength="250" value="<%=topic%>"> <font color="#FF0000"> *</font>
	
	&nbsp;&nbsp;&nbsp;附加一张形象照片：
	<INPUT type="radio" name="joinphoto" id="joinphoto_0" title="我要附加一张形象照片，作为外教突出显示的照片！" onClick="document.all.logPhotoAddressURL_Temp.style.display='block';" <% If logPhoto_AddressURL<>"" And Len(logPhoto_AddressURL)>6 Then Response.Write("checked") %>>
	<label for="joinphoto_0">是</label>
	&nbsp;
	<INPUT type="radio" name="joinphoto" id="joinphoto_1" title="不是发布外教个人资料信息，不用附加形象照片." onClick="document.all.logPhotoAddressURL_Temp.style.display='none';" <% If logPhoto_AddressURL="" Or Len(logPhoto_AddressURL)<6 Then Response.Write("checked") %>>
	<label for='joinphoto_1'>否</label>
	

	</ul>
	
		
	<ul id="logPhotoAddressURL_Temp" class="list_edit" style="background:#eeeeee;padding:3px; display:<% If (logPhoto_AddressURL="" Or Len(logPhoto_AddressURL)<6) and isTcomehere=0 Then Response.Write("none") %>;">
	
		可附加文章相片地址：
	<input name="logPhoto_AddressURL" type=text id="logPhoto_AddressURL" size="40" maxlength="250" value="<%=logPhoto_AddressURL%>" <% If logPhoto_AddressURL<>"" And Len(logPhoto_AddressURL)>6 Then Response.Write("onclick=window.open('"& logPhoto_AddressURL &"')") %>>
	
	<% If logPhoto_AddressURL<>"" And Len(logPhoto_AddressURL)>6 Then %>
		&nbsp;&nbsp;&nbsp;预览：
		<a href="<%=logPhoto_AddressURL%>" target="_blank">
		<img title="您可以在此看到是否能正常看到图片！" src="<%=logPhoto_AddressURL%>" width="50" height="50" /></a>
	<% End If %>
		...
		一般情况下不填写！
		<br />（格式例如：<font color="#038ad7">http://www.myhomestay.com.cn/images/abcdef.jpg</font>  或者去掉域名：<font color="#038ad7">/images/abcdef.jpg</font>）
		
		<br />
		------------------------------
		<input name="isTcomehere" type="radio"  value="1" <% If isTcomehere=1 Then Response.Write("checked") %>>使用外教备注并记入时间表中；
		<input name="isTcomehere" type="radio"  value="0" <% If isTcomehere=0 Then Response.Write("checked") %>>   不使用外教备注------------------------------
		<br />
		
		到京时间：---<input name="TcomehereDate" type="text" id="TcomehereDate" size="30" maxlength="50" value="<%=TcomehereDate%>" >
		<br />
		
		姓名：--------<input name="TName" type="text" id="TName" size="30" maxlength="50" value="<%=TName%>" >
		<br />
		
		打算几个月：<input name="TcomehereHowLongTime" type="text" id="TcomehereHowLongTime" size="30" maxlength="50" value="<%=TcomehereHowLongTime%>" >
		<br />
		
		目的地：------<input name="TcomehereWhither" type="text" id="TcomehereWhither" size="30" maxlength="50" value="<%=TcomehereWhither%>" >
		<br />
		
		国籍：--------<input name="TNationality" type="text" id="TNationality" size="30" maxlength="50" value="<%=TNationality%>" >
		<br />
	...
	文章分类：
	<%	
		Dim sql_Temp01,rs_Temp01,logclassid_Nav_Temp
		Set rs_Temp01=Server.CreateObject("Adodb.RecordSet")
		sql_Temp01="Select logclassid_Nav From oblog_user Where userid=" &oblog.logined_uid
		if not IsObject(conn) then link_database
		rs_Temp01.Open sql_Temp01,Conn,1,3
			logclassid_Nav_Temp =rs_Temp01(0)
		rs_Temp01.Close
		set rs_Temp01=nothing
		'response.Write logclassid_Nav_Temp
	%> 
	<select name="classid" id="classid">	
	<%'=oblog.show_class("log",classid,t)%>
	
	<%=oblog.show_class_Nav("log",classid,t,logclassid_Nav_Temp)%>
...
%>				
	...
	<input type="submit" value="发布<%=tName%>" />
	<%if t="0" then%>
...
sub savelog()
	...
	logPhoto_AddressURL = Trim(Request.Form("logPhoto_AddressURL"))'WL;
	
	isTcomehere= Request.Form("isTcomehere")'是否记录进 ‘外教到来时间表’？
	TcomehereDate= Request.Form("TcomehereDate")'外教到来时间表
	TName= Request.Form("TName")'外教的姓名
	TcomehereHowLongTime= Request.Form("TcomehereHowLongTime")'外教到来打算住几个月
	TcomehereWhither= Request.Form("TcomehereWhither")'外教目的地（如海淀区、sohu现代城附近）
	TNationality= Request.Form("TNationality")'外教的国籍
...
'	End If	
	...
	'''If logPhoto_AddressURL<>"" Then
	rs("logPhoto_AddressURL")= logPhoto_AddressURL'WL;
	'''End If
	
	If isNumeric(isTcomehere) Then
		If isTcomehere=1 Then
		rs("isTcomehere")= isTcomehere'WL;
			rs("TcomehereDate")= TcomehereDate'WL;
			rs("TName")= TName'WL;
			rs("TcomehereHowLongTime")= TcomehereHowLongTime'WL;
			rs("TcomehereWhither")= TcomehereWhither'WL;
			rs("TNationality")= TNationality'WL;
		Else
			rs("isTcomehere")= 0'WL;
			rs("TcomehereDate")= ""'WL;
			rs("TName")= ""'WL;
			rs("TcomehereHowLongTime")= ""'WL;
			rs("TcomehereWhither")= ""'WL;
			rs("TNationality")= ""'WL;
		End If
	End If
	...
	if isdraft=0  then
		...
		if log_modiid=0 then
			set rs=oblog.execute("select top 1 logid from oblog_log where logid<"&tid&" and userid="&log_blogteam&" and logtype="&cint(t)&" order by addtime desc")
		...
		response.Write("<li><a href=""user_post.asp?logid="&tid&""">已经保存到草稿箱，点击继续修改文章</a></li>")
	end if
	'发布或修改文章后重新生成功能页面
	set rs=nothing
	set blog=nothing
